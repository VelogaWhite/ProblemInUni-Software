...
from .models import Issue, IssueStatusLog, Notification, HotspotCoordinate
...
def issue_detail(request, issue_id):
    ...
    # เรียงลำดับประวัติตามเวลา เพื่อให้แสดงใน Timeline ได้ถูกต้อง
    logs  = issue.status_logs.all().order_by('changed_at')
    notif_count = request.user.notifications.filter(is_read=False).count() if request.user.is_authenticated else 0
    
    # 🌟 ดึงข้อมูลพิกัด 3D Hotspot เฉพาะที่ตรงกับโซนของปัญหานี้
    hotspots = HotspotCoordinate.objects.filter(zone=issue.zone) if issue.zone else []
    
    return render(request, 'issues/issue_detail.html', {
        'issue': issue, 
        'logs': logs, 
        'notif_count': notif_count,
        'hotspots': hotspots, # 🌟 ส่งข้อมูลพิกัดไปให้ Template ใช้งาน
    })
...
def coordinate_finder(request):
    return render(request, 'issues/coordinate_finder.html')